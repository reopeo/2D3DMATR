...
def get_2d3d_node_correspondences(
    ...
    # Early return when no candidates exist (e.g. sparse outdoor depth maps).
    # KeOps raises CUDA_ERROR_INVALID_VALUE when asked to allocate 0 bytes.
    if num_candidates == 0:
        empty_idx = candidate_img_indices  # shape (0,), dtype long
        empty_ov = torch.zeros(0, device=transform.device)
        return empty_idx, empty_idx, empty_ov, empty_ov
