...
class CoarseMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict):
        ...
        # Guard: circle_loss calls .mean() on tensors filtered by row_masks / col_masks.
        # row_masks requires each row to have ≥1 positive AND ≥1 negative pair.
        # When pos_masks is all-False (no valid node correspondences, e.g. sparse
        # outdoor depth), row_masks is all-False → mean() of empty tensor → nan.
        row_masks = torch.gt(pos_masks.sum(-1), 0) & torch.gt(neg_masks.sum(-1), 0)
        if not row_masks.any():
            return (img_feats * 0.0).sum()
...
class FineMatchingLoss(nn.Module):
    ...
    def forward(self, data_dict, output_dict):
        ...
        # Guard: no correspondences (e.g. sparse outdoor depth with no overlap)
        if pcd_corr_indices.shape[0] == 0:
            zero_loss = (img_feats * 0.0).sum()  # keeps gradient graph alive
            zero_recall = img_feats.new_zeros(1).squeeze()
            return zero_loss, zero_recall
