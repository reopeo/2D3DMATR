...
class MATR2D3D(nn.Module):
    def __init__(self, cfg):
        ...
        # depth_limit for back-projecting the synthetic depth map (metres)
        self.depth_limit = cfg.model.depth_limit
    ...
    def forward(self, data_dict):
        ...
        img_points, img_masks = back_project(
            depth, intrinsics, depth_limit=self.depth_limit, transposed=True, return_mask=True
        )
        ...
        if not self.training:
            (
                ...
                node_corr_scores,
            ) = self.coarse_matching(img_feats_c, pcd_feats_c, img_node_masks, pcd_node_masks)
